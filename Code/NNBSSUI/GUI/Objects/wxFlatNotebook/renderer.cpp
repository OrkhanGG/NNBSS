...
#include "headers/wxFlatNotebook.h"// for styles
#include "headers/fnb_resources.h"
#include <wx/image.h>
...
void wxFNBRenderer::DrawTabX(wxWindow* pageContainer, wxDC& dc, const wxRect& rect, const int& tabIdx, const int btnStatus)
{
	...
	{
	case wxFNB_BTN_HOVER:
		xBmp = NNBSS_PNG2BMP(NNBSSART_GLOBAL_X_16x16p_HOVER);
		break;
	case wxFNB_BTN_PRESSED:
		xBmp = NNBSS_PNG2BMP(NNBSSART_GLOBAL_X_16x16p_HOVER);
		break;
	case wxFNB_BTN_NONE:
	default:
		xBmp = NNBSS_PNG2BMP(NNBSSART_GLOBAL_X_16x16p_IDLE);
		break;
	}
	...
	// Draw the new bitmap
	dc.DrawBitmap(xBmp, rect.x, rect.y - 1, true);
...
void wxFNBRenderer::DrawTabsLine(wxWindow* pageContainer, wxDC& dc, wxCoord selTabX1, wxCoord selTabX2)
{
	...
	if (pc->HasFlag(wxFNB_FF2)) {
		...
		dc.SetBrush(*wxTRANSPARENT_BRUSH);
		//dc.SetPen( wxPen(pc->m_SingleLineBorderColour,1) );
		//dc.DrawLine(0, clientRect.GetBottom()+2, clientRect.GetRight(), clientRect.GetBottom()+2);
		//dc.DrawRectangle(clientRect2);
		//dc.DrawRectangle(clientRect3);

		dc.SetPen(wxPen(pc->m_SingleLineBorderColour));
		//dc.DrawRectangle(clientRect); // NNBSS CHANGE: DRAW RECTANGLE ALONG TAB AREA

		//int _PageContainer_Height = pc->_TabAdditionalHeight == 0 ? pageContainer->GetSize().y
		//	: pc->_TabAdditionalHeight;
		//dc.DrawLine(clientRect.GetTopLeft().x, clientRect.GetTopLeft().y, clientRect.GetTopRight().x, clientRect.GetTopRight().y); // NNBSS CHANGE : I underline the tab with a pen with tab area colour
		// TODO: tab height may not overlapping with tab height. It's because additional tab height. So, if causes any problem, find an accurate way.
		dc.DrawLine(clientRect.GetBottomLeft().x, clientRect.GetBottomLeft().y + 2, clientRect.GetBottomRight().x, clientRect.GetBottomRight().y + 2); // NNBSS CHANGE : I underline the tab with a pen with tab area colour

		/*if( !pc->HasFlag(wxFNB_TABS_BORDER_SIMPLE) )
		{
			dc.SetPen(wxPen( pc->HasFlag( wxFNB_VC71) ? wxColour(247, 243, 233) : pc->m_SingleLineBorderColour));
...
int wxFNBRenderer::CalcTabWidth(wxWindow* pageContainer, int tabIdx, int tabHeight)
{
	...
	if (pc->IsDefaultTabs())
		// Default style
		tabWidth += 2 * shapePoints + pc->_TabAdditionalWidth;// NNBSS CHANGE: I extend the width a bit more
...
int wxFNBRenderer::CalcTabHeight(wxWindow* pageContainer)
{
	...
	int _PageContainer_Height = pc->_TabAdditionalHeight == 0 ? pageContainer->GetSize().y
		: pc->_TabAdditionalHeight;
	pageContainer->SetSize(wxSize(pageContainer->GetSize().x, _PageContainer_Height));
	...
	tabHeight += pageContainer->GetSize().y - 25; // NNBSS CHANGE: -25 to equalize bottom to pageContainer
#ifdef __WXGTK__
...
void wxFNBRenderer::DrawTabs(wxWindow* pageContainer, wxDC& dc, wxEvent& event)
{
	...
	if (pc->HasFlag(wxFNB_FF2)) {
		...
		dc.SetPen(wxPen(*wxTRANSPARENT_PEN));// no border colour on rectangle
		dc.DrawRectangle(0, 0, size.x, size.y);
	}
	...
	// We always draw line at the bottom and top of the tabs
	// regardless the style
	dc.SetPen(borderPen);
	...
	for (i = pc->m_nFrom; i < (int)pc->GetPageInfoVector().GetCount(); i++)
	{
		...
		wxFont boldFont = normalFont;
		boldFont.SetWeight(wxFONTWEIGHT_BOLD);
		dc.SetFont((i == pc->GetSelection()) ? boldFont : normalFont);// NNBSS CHANGE: selected and not selected tabs should have the same font!
...
void wxFNBRendererDefault::DrawTab(wxWindow* pageContainer, wxDC& dc,
	const int& posx, const int& tabIdx, const int& tabWidth,
	const int& tabHeight, const int btnStatus)
{
	// 
		// Default style
	wxPen borderPen = wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNSHADOW));
	...
	{
		dc.SetBrush(wxBrush(RGB_COLOURDEF_TRIPLE20));
		dc.SetPen(wxPen(RGB_COLOURDEF_PLACEHOLDER));
		// Draw the tab as rounded rectangle
		dc.DrawPolygon(7, tabPoints);
		// set the brush to transparent again
		dc.SetBrush(*wxTRANSPARENT_BRUSH);
	}
	else
	{
		...
		dc.SetPen(wxPen(RGB_COLOURDEF_PLACEHOLDER));
		dc.DrawPolygon(7, tabPoints);
		// if you don't want to draw near to selection, then use this condition
		// btw, the code below is for not to draw polygons on unselected tabs
			/*
		if(tabIdx != pc->GetSelection() - 1)
		{
			...
			pt1x = tabPoints[5].x;
			pt1y = pc->HasFlag(wxFNB_BOTTOM) ? 4 : tabHeight; // old val was (tabHeight - 6)
			pt2x = tabPoints[5].x;
			pt2y = pc->HasFlag(wxFNB_BOTTOM) ? tabHeight - 4 : 2 ;// old val was 4
			dc.DrawLine(pt1x, pt1y, pt2x, pt2y); // like separator. but, doesn't look so good.
		}
			*/
	}
	...
	if (tabIdx == pc->GetSelection())
	{
		...
		wxPen tabUnderlinePen = wxPen(RGB_COLOURDEF_NNBSSBLUEHIGHLIGHT);// NNBSS CHANGE: change to pc->m_activeTabColor not to underline selected tabs
		tabUnderlinePen.SetWidth(2);
		dc.SetPen(tabUnderlinePen);

		wxPoint secPt = wxPoint(tabPoints[5].x, tabPoints[5].y);
	...
	//dc.DrawText(pc->GetPageText(tabIdx), posx + textOffset + pc->_TabAdditionalWidth/2,
	//	imageYCoord-2); // NNBSS CHANGE: tabs need to be enlarged in y direction. so text have to be changed too!

	dc.DrawText(pc->GetPageText(tabIdx), posx + textOffset + pc->_TabAdditionalWidth / 2,
		tabHeight / 2 - imageYCoord / 2);// NNBSS CHANGE: instead of old method, this'll help us to center the text in vertical direction
	...
	if (pc->HasFlag(wxFNB_X_ON_TAB) && tabIdx == pc->GetSelection())
	{
		...
		//int tabCloseButtonXCoord = posx + textOffset + textWidth + 1; // NNBSS CHANGE: old method
		int tabCloseButtonXCoord = posx + textOffset + textWidth + 1 + pc->_TabAdditionalWidth;// NNBSS CHANGE: new method to place X on tab

		// take a bitmap from the position of the 'x' button (the x on tab button)
		// this bitmap will be used later to delete old buttons
		//int tabCloseButtonYCoord = imageYCoord;// NNBSS CHANGE: Old method
		int tabCloseButtonYCoord = tabHeight / 2 - imageYCoord / 2; // NNBSS CHANGE: New method
