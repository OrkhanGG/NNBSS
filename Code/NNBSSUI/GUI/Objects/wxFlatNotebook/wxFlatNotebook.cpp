...
// wxFlatNotebook
#include "headers/wxFlatNotebook.h"
#include "headers/renderer.h"
#include "headers/popup_dlg.h"
#include "headers/fnb_customize_dlg.h"
                             ...
                             long style, const wxString& name)
{
	...
	int tabHeight = 
	height + wxFNB_HEIGHT_SPACER + 
	(m_pages->_TabAdditionalHeight - (height + wxFNB_HEIGHT_SPACER));// NNBSS CHANGE: To move down 
														 //the content because of additional Height 
#ifdef __WXGTK__
	...
	m_mainSizer->Insert(0, m_pages, wxSizerFlags().Proportion(0).Expand());
	m_mainSizer->Layout();
...
void wxFlatNotebook::DeletePage(size_t page, bool notify)
{
	...
	if (pageRemoved)// if is not destroyed already,
	{
		pageRemoved->Destroy();
		pageRemoved = nullptr;
	}
...
void wxFlatNotebook::OnNavigationKey(wxNavigationKeyEvent& event)
{
	...
	else
	{
		...
		if ( GetParent() )
		{
			...
			// for now disabled //GetParent()->ProcessEvent(event);
		}
	}
}
...
void wxFlatNotebook::SetSingleLineBorderColor(wxColor borderColor)
{
	if (HasFlag(wxFNB_FANCY_TABS)) {
		wxLogError("SingleLineBorderColor can't be changed when wxFNB_FANCY_TABS is enabled!");
		return;
	}
	m_pages->m_SingleLineBorderColour = borderColor;
...
wxColor wxPageContainer::GetSingleLineBorderColor()
{
	if(HasFlag(wxFNB_FANCY_TABS))
		return m_colorFrom;
	return m_SingleLineBorderColour;
}
